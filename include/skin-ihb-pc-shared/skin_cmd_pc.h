 ...
 * and the IHB.
 *
 * When adding commands, set its value to the maximum present plus 1 and make
 * sure not to change the other commands.
 */
...
enum SkinCmd {
  ...
  /* set whether SYNCs are required for each period, or IHB automatically
     handles the skin */
  ...
  /* SPI speed during normal operation (argument is valus in Mbps: 1, 2, 3, 4 or
     8) */
   ...
   **************************/
  /* when sync is received, the IHB sends a command to the STs to start
     acquiring */
  SKIN_PATCH_CMD_SYNC = 0x3B,
  /* declare number of modules and number of sensors in each module */
  SKIN_PATCH_CMD_DECLARE = 0x3C,
  /* get details of the module, such as sensor groupings, which sensors are cut
     etc */
   ...
   **************************/
  /*
   * the sensor data will be divided up in fractions, since they don't all fit
   * in one CANCMD message, and sending each module individually will also have
   * considerable overhead.  The sensor data should thus be marked with two
   * bytes in the beginning that say which module and which sensor in that
   * module the first response in this chunk correspond to.
   */
   ...
   *    debug operations    *
   **************************/
  /* disable, disable excitation or enable CDC of a module, making it react to
     or ignore syncs */
